...
mxShape.prototype.getShadowStyle = function () {
  ...
  if (this.style != null) {
    s.dx = mxUtils.getValue(this.style, mxConstants.STYLE_SHADOW_OFFSET_X, s.dx)
    s.dy = mxUtils.getValue(this.style, mxConstants.STYLE_SHADOW_OFFSET_Y, s.dy)
    s.blur = mxUtils.getValue(this.style, mxConstants.STYLE_SHADOW_BLUR, s.blur)
    s.color = mxUtils.getValue(this.style, mxConstants.STYLE_SHADOWCOLOR, s.color)
    s.opacity = mxUtils.getValue(this.style, mxConstants.STYLE_SHADOW_OPACITY, s.opacity)
  }
...
mxShape.prototype.createDropShadow = function (style, scale) {
  return (
    'drop-shadow(' +
    Math.round(style.dx * scale * 100) / 100 +
    'px ' +
    Math.round(style.dy * scale * 100) / 100 +
    'px ' +
    Math.round(style.blur * scale * 100) / 100 +
    'px ' +
  ...
  if (pts != null) {
    ...
    if (pts.length > 0) {
      ...
      for (var i = 1; i < pts.length; i++) {
        ...
        if (Math.abs(p0.x - pe.x) >= t || Math.abs(p0.y - pe.y) >= t) {
