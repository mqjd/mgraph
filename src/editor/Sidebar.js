...
import { STENCIL_PATH, urlParams } from './Init'
...
Sidebar.prototype.refresh = function () {
  ...
  this.graph.stylesheet.styles = mxUtils.clone(graph.getDefaultStylesheet().styles)
...
Sidebar.prototype.wasPaletteExpanded = function (paletteStates, id, defaultExpanded) {
  ...
  if (elts != null && elts.length == 2 && elts[1].firstChild != null) {
      ...
      if (normalized.length > 0) {
        ...
        if (arr.length > 0) {
          ...
          for (var j = 0; j < arr.length; j++) {
            ...
            if ((index == 0) == (dict.get(entry) == null) && tmpDict.get(entry) == null) {
                  ...
                  for (var i = 0; i < results.length; i++) {
                    mxUtils.bind(this, function (result) {
                      try {
                        ...
                        if (this.closedLibraryOpacity != null && this.searchClosedLibraries) {
...
Sidebar.prototype.addUmlPalette = function (expand) {
  ...
  var lifelineStyle =
    'shape=umlLifeline;perimeter=lifelinePerimeter;whiteSpace=wrap;html=1;container=1;dropTarget=0;' +
    'collapsible=0;recursiveResize=0;outlineConnect=0;portConstraint=eastwest;' +
    sequenceEdgeStyle
  var activationStyle =
    'html=1;points=[[0,0,0,0,5],[0,1,0,0,-5],[1,0,0,0,5],[1,1,0,0,-5]];perimeter=orthogonalPerimeter;' +
    'outlineConnect=0;targetShapes=umlLifeline;portConstraint=eastwest;' +
    sequenceEdgeStyle
  ...
  var fns = [
    ...
    this.addEntry(dt + 'item member method function variable field attribute label', function () {
      var cell = new mxCell(
        ...
        'label;fontStyle=0;strokeColor=none;fillColor=none;align=left;verticalAlign=top;overflow=hidden;' +
          'spacingLeft=28;spacingRight=4;rotatable=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;imageWidth=16;imageHeight=16;whiteSpace=wrap;html=1;image=' +
          sb.gearImage
      )
      cell.vertex = true
    ...
    this.addEntry(dt + 'component', function () {
      var cell = new mxCell(
        '<p style="margin:0px;margin-top:6px;text-align:center;"><b>Component</b></p>' +
          hr +
          '<p style="margin:0px;margin-left:8px;">+ Attribute1: Type<br/>+ Attribute2: Type</p>',
        new mxGeometry(0, 0, 180, 90),
    ...
    this.createVertexTemplateEntry(
      ...
      160,
      90,
      '<p style="margin:0px;margin-top:4px;text-align:center;text-decoration:underline;"><b>Object:Type</b></p>' +
        hr +
    ...
    this.addEntry(dt + 'object instance', function () {
      var cell = new mxCell(
        '<p style="margin:0px;margin-top:4px;text-align:center;">' +
          ...
          hr +
          '<p style="margin:0px;margin-left:4px;">+ field1: Type<br/>' +
          '+ field2: Type</p>' +
          hr +
    ...
    this.createVertexTemplateEntry(
      lifelineStyle + 'participant=umlEntity;',
      40,
      300,
      '',
      'Entity Lifeline',
    ...
    this.createVertexTemplateEntry(
      lifelineStyle + 'participant=umlControl;',
      40,
      300,
      '',
      'Control Lifeline',
...
Sidebar.prototype.createThumb = function (
  ...
  this.graph.shapeBackgroundColor = bg != null ? bg : Editor.isDarkMode() ? '#2a252f' : '#f1f3f4'
  ...
  if (cells != null && cells.length > 0) {
    ...
    if (cells.length > 1 || cells[0].vertex) {
      var ds = this.createDragSource(
        elt,
        this.createDropHandler(cells, true, allowCellsInserted, bounds, startEditing, sourceCell),
      ...
      var ds = this.createDragSource(
        elt,
        this.createDropHandler(cells, false, allowCellsInserted, bounds, startEditing, sourceCell),
        this.createDragPreview(width, height),
        cells,
        bounds,
        startEditing
      ...
      if (cells.length > 0) {
        ...
        if (!graph.isCellLocked(target || graph.getDefaultParent())) {
          ...
          try {
            ...
            for (var i = 0; i < select.length; i++) {
              if (graph.model.isVertex(select[i]) && graph.isAutoSizeCell(select[i])) {
...
Sidebar.prototype.disablePointerEvents = function (node) {
  mxUtils.visitNodes(
    node,
    mxUtils.bind(this, function (node) {
...
Sidebar.prototype.createDragSource = function (
  ...
  dragSource.getDropTarget = mxUtils.bind(this, function (graph, x, y, evt) {
    ...
    if (target != null && (activeArrow != null || !graph.isSplitTarget(target, cells, evt))) {
