        ...
        var addItem = mxUtils.bind(this, function (fontFamily) {
          var tr = this.styleChange(
            ...
            function () {
              ...
              ui.fireEvent(
                new mxEventObject('styleChanged', 'keys', [mxConstants.STYLE_FONTFAMILY], 'values', [fontFamily], 'cells', [
      ...
      mxUtils.bind(this, function (menu, parent) {
        ...
        var setFontSize = mxUtils.bind(this, function (fontSize) {
          if (graph.cellEditor.textarea != null) {
            ...
            ui.fireEvent(
              new mxEventObject('styleChanged', 'keys', [mxConstants.STYLE_FONTSIZE], 'values', [fontSize], 'cells', [
                graph.cellEditor.getEditingCell()
              ])
            )
          }
      ...
      if (cmd != null && graph.cellEditor.isContentEditing()) {
        ...
        var dlg = new ColorDialog(
          ...
          mxUtils.bind(this, function (color) {
            ...
            if (style != null) {
              ui.fireEvent(new mxEventObject('styleChanged', 'keys', [style], 'values', [color], 'cells', [graph.cellEditor.getEditingCell()]))
            }
          }),
          function () {
            graph.cellEditor.restoreSelection(selState)
        ...
        if (defaultColor != null) {
          ...
          this.editorUi.pickColor(color, ColorDialog.createApplyFunction(this.editorUi, key), defaultColor, defaultColorValue)
        } else {
...
Menus.prototype.addPopupMenuCellItems = function (menu, cell, evt) {
  ...
  if (state != null) {
    ...
    if (this.isShowStyleItems() && graph.getSelectionCount() == 1 && graph.getModel().isEdge(cell)) {
      menu.addSeparator()
...
Menubar.prototype.addMenuHandler = function (elt, funct, clickFn) {
  if (funct != null) {
    ...
    mxEvent.addListener(
      ...
      'mousemove',
      mxUtils.bind(this, function (evt) {
        if (this.editorUi.menus.autoPopup && this.editorUi.currentMenu != null && this.editorUi.currentMenuElt != elt) {
          this.editorUi.hideCurrentMenu()
          clickHandler(evt)
        }
      })
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function (evt) {
        if (!this.editorUi.menus.autoPopup && this.editorUi.currentMenu != null && this.editorUi.currentMenuElt != elt && mxEvent.isMouseEvent(evt)) {
          this.editorUi.hideCurrentMenu()
        }
